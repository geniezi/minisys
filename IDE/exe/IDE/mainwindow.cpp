...
void MainWindow::on_actionSave_triggered()
{
    if(this->hasOpenFile){
     ...
     {
         QString filename=this->currentFilePath;
...
//                ui->textEdit->insertPlainText(text +='\n');
    ...
    else{QString filename=QFileDialog::getSaveFileName(this,tr("选择保存路径"),"Untitled","C File(*.c);;C++ File(*.cpp);;Header File(*.h)");
            if(filename.isEmpty())
            {
...
void MainWindow::on_actionOpenFile_triggered()
{
    if(this->hasOpenFile){
        if(QMessageBox::Yes==  QMessageBox::question(this,"提示","打开新的文件前是否保存当前文件？"))
           {
            QString filename=QFileDialog::getSaveFileName(this,tr("选择保存路径"),this->currentFileName,tr("C File(*.c);;C++ File(*.cpp);;Header File(*.h)"));
                    if(filename.isEmpty())
                    {
                        QMessageBox::information(this,"提示","文件路径不能为空");
                        return;
...
//                        qDebug() << file.open(QIODevice::ReadWrite);
                    ...
                    filename = QFileDialog::getOpenFileName(this, " 选择需要打开的文件 " , "" , " *.c *.cpp *.h" );
        ...
        QString filename = QFileDialog::getOpenFileName(this, " 选择需要打开的文件 " , "" , " *.c *.cpp *.h" );
...
void MainWindow::on_actionRun_triggered()
{
    if(this->hasOpenFile)
    {
      ...
      if(this->currentFilePath.isEmpty())
    {
     QString filename=QFileDialog::getSaveFileName(this,tr("选择保存路径"),"Untitled",tr("C File(*.c);;C++ File(*.cpp);;Header File(*.h)"));
                    if(filename.isEmpty())
                    {
                        QMessageBox::information(this,"提示","文件路径不能为空");
                        return;
...
//          QMessageBox::information(this,"提示","保存执行");
...
//    qDebug() << "编译执行";
//    QMessageBox::information(this,"提示","编译执行");
    ...
    if(tag)
     {
        QByteArray array =  newfile.readAll();
        QString o_message=codec->toUnicode(array);
        qDebug()<<o_message;
        QMessageBox::information(this,"编译提示",o_message);
         return;
     }
...
//    QMessageBox::information(this,"提示","汇编执行");
