...
bool SeuYacc::readGrammer(const string grammerFile) {
	...
	while (!endFlag) {
			...
			if (isLegalLetter(_peek)) {
				...
				if (!lexeme.empty()) {
					...
					if (found != _nonterminalTable.end()) {
						...
						_nonterminalTable.insert(make_pair(lexeme, _indexSymbol));
						_nonterminalNullable.insert(make_pair(_indexSymbol, false)); //默认不是空串
						_word.push_back(lexeme);
				...
				if (list.empty()) {
					// empty production，产生式为空串，记录该非终结符为空串
					_nonterminalNullable.find(procHead)->second = true;
				}
				//list.swap(list<ElementType>());// 清空list列表
				list.clear();
				lexeme = "";
			}
			else if (_peek == '\'' || _peek == ';' || _peek == '{') {
		...
		case 2:
			...
			break;
		case 3:
			// add production，类似读到|，将之前保存的产生式保存到_productionVector
			_productionVector.push_back(ProductionItem(procHead, list, list.size(), numOfProd++));
			if (list.empty()) {
				// empty production
				_nonterminalNullable.find(procHead)->second = true;
			...
			break;
		case 4:
			// action part 定义动作，用于语法制导翻译生成中间代码
			// yacc.y文件中{}中的部分
			read();
	...
	//所有语法读取完毕
	translateAction(); //把读取到动作由string转换成可执行动作
	nullableAnalyze(); //记录非终结符是否产生空串
	_terminalTable.insert(make_pair("$", _indexSymbol)); //$作为特殊的终结符
	_word.push_back("$");
	++_indexSymbol;
...
bool SeuYacc::readPriority() {
	...
	while (true) {
		...
		else if (_peek == '%') {  //附加条件读取完
			return true;   
		}
	}
}

// read a char from sourse file，读取一个字符，读取完之后换行
bool SeuYacc::read() {
	if (_offset == -1) {
		if (!_sourcefile.eof()) {
...
void SeuYacc::initTransition() {
	...
	_LR1stateTransition.push_back(TransitionItem(LRClosure(H), 0)); // 产生相应的闭包
	// 状态边的扩展
	// inner-state-extension and between-state-extension
	IDType maxState = 1;
	for (IDType k = 0; k < maxState; ++k) { //从0状态到最大状态开始遍历
			...
			if (p._dot == p._prod->_bodyList.end()) { //·移到了最后需要进行规约
				for (auto& pred : p._predSet) { //填入预测符的规约项
					...
					case ERROR: //TableItem初始化空格子都为ERROR
						...
						if (p._prod->_index == 0) //第0条表达式即开始符号S'
...
bool SeuYacc::transformIntoLALR() {
	...
	for (auto it : mergeMap) {
		// 取出mergeMap相同核状态的第一个state的LR1state
	...
	for (IDType curstate = 0; curstate != _LR1parseTable.size(); ++curstate) {
		...
		// 对于LR1parseTable每一个状态中的每一项修改SHIFT和GOTO_STATE跳到的状态
		...
		if (found == mergeMap.end()) {
			...
			// 将表格上状态相同的两行进行合并
			auto mergeFinalTranItr = _LALR1parseTable.at(found->second).begin();  // 取出最终保留在LALR1parseTable的一行
...
void SeuYacc::outputAction() {
	...
	for (unsigned int index = 0; index < _productionAction.size(); ++index) {
		...
		out << "\treturn pair<unsigned int, string>(" 
			<< _productionVector[index]._bodyLength << ",\"" //产生式右部长度
